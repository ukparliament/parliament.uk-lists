module Groups
  class MembershipsController < ApplicationController
    ...
    def index
      @group, @people, @chair_people, @letters = Parliament::Utils::Helpers::FilterHelper.filter(@request, 'Group', 'Person', 'Position', ::Grom::Node::BLANK)
      @group = @group.first
      @people = @people.sort_by(:sort_name)
      @chair_people.map! { |chair| chair.incumbencies.map(&:people) }.flatten!.uniq!
      @non_chair_members = Array(@people) - Array(@chair_people)
      @letters = @letters.map(&:value)
    end

    def current
      @group, @people, @chair_people, @letters = Parliament::Utils::Helpers::FilterHelper.filter(@request, 'Group', 'Person', 'Position')
      @group = @group.first
      @people = @people.sort_by(:sort_name)
      @chair_people.map! { |chair| chair.incumbencies.map(&:people) }.flatten!.uniq!
      @non_chair_members = Array(@people) - Array(@chair_people)
